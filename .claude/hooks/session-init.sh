...
if [ "$LOOP_SESSIONS" -gt 0 ]; then
  ...
  if [ -f "$PROJECT_PATH/.claude/skills/run-loop/scripts/warn-stale.sh" ]; then
    bash "$PROJECT_PATH/.claude/skills/run-loop/scripts/warn-stale.sh"
  fi
fi
...
if [ -n "$MISSING" ]; then
  ...
  echo "  bd:   see https://github.com/hwells4/beads"
fi
